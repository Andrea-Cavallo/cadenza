...
- **Debug logging throughout LLM generation pipeline**: all new logs use `slog.Debug` so they are silent at the default Info level and visible when `--log-level debug` is set.
  ...
  - `repair.go`: logs every individual fix applied — BPM/Bars clamp, SpecVersion default, style_profile substitution, evolution action fix (from→to) and intensity scaling, per-step note octave shift (from_note→to_note with MIDI values), density fill/deactivate per step, chord coherence percentage per section and each chord-tone replacement (from→to, chord name).

### Fixed
- **`unknown style_profile "bassline"` repair failure**: LLMs sometimes output the pattern type name (e.g. "bassline") as the `style_profile` value. The repair pipeline now calls `repairStyleProfile` before re-validating, replacing any unrecognised profile with the correct type default (`bass_progressive`, `arp_flowing`, `melody_expressive`).
- **Evolution action/intensity repair**: `repairEvolution` now fixes invalid action strings by mapping them to a positional default (introduce→build→peak→release), scales out-of-range intensities (LLM sometimes outputs 0–100 instead of 0.0–1.0), and inserts a default 4-phase arc when the evolution array is empty.
- **Repair skipping BPM/Bars/SpecVersion validation errors**: `repairMetaFields` now clamps BPM to [80, 150], replaces invalid bar counts (anything not 4/16/32/64/128) with the value from `MusicContext`, and defaults `spec_version` to "1.0" when absent — ensuring the spec passes the meta checks before musical content is re-validated.
- **LLM melody generates bass-range notes (A2/E2)**: `exampleSpecJSON` was pattern-type-blind — it showed A2/E2 bass notes as the example JSON for all three pattern types. The LLM copied those octaves into melody output. Fixed by making `exampleSpecJSON` return pattern-type-specific examples: melody uses A4/E5 (octave 4-6), arpeggio uses A3/E4/C5 (octave 3-6), bassline keeps A2/E2 (octave 1-3). All three examples have correct density and pass anti-loop checks.
- **Chord coherence correction examples used wrong chord (hardcoded Fm)**: when the LLM failed with "found 1/3 chord tones of Bm", the retry correction always showed Fm notes (F3, Ab3, C4), actively confusing the LLM on subsequent attempts. Now the validator embeds the real chord tones in the error message (`; chord tones: B, D, F#`) and the correction example uses those actual tones at the correct octave for the pattern type.
- **No repair path after 3 retries**: after `GenerateWithRetry` exhausted all retries the only option was the offline template. Added `RetryExhaustedError` that carries the last invalid JSON, and a programmatic `repairSpec` function in the generator that fixes note octaves (shift by octave until in range), density (activate spread across sections to also fix anti-loop), and chord coherence (replace non-chord-tone notes with the nearest chord tone in range). If repair passes validation it is used instead of falling back to offline.
- **Build script macOS desktop check on Windows**: `Assert-PackageContents` incorrectly required a desktop folder for darwin/universal packages even when Wails could not cross-compile from Windows. Fixed by deriving `$desktopExpected` from the build result string instead of from the requested platform list.
