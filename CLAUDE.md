...
| `backend/internal/session/` | Session persistence: `.czs` files (32-byte header + JSON), `SessionStore` interface, SHA1 file naming, FIFO eviction |
...
4. `golangci-lint run ./...` — must pass with zero `Error:` lines
...
5. `govulncheck ./...` — must return zero vulnerability findings
...
10. **`CLAUDE.md`** (this file) — update whenever: architecture changes, a new invariant is introduced, a convention changes, a new directory is added
...
13. If `go.mod` Go version changes: update `.golangci.yml` `run.go` to match **in the same change**
